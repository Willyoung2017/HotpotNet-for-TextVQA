...
def main():
    ...
    if args.task == "update":
        ...
        for k,v in models.items():
            pred[k] = {i:pd.read_json("./model_predictions/{}/textvqa-val-{}.json".format(k,i)) for i in v}
        ...
        # (6) generate gradebooks with predictions for all-zero questions, and worst performing questions
        max = accuracies.max(axis=1)
        min = accuracies.min(axis=1)
        sec_min = accuracies.apply(lambda row: row.nsmallest(2).values[-1], axis=1)

        all_zero_gradebook_with_pred = gradebook_with_pred[max+min==0]

        unique_zero_gradebook = gradebook[(min==0) & (sec_min>0)]
        unique_zero_gradebook_with_pred = gradebook_with_pred[(min==0) & (sec_min>0)]
        unique_zero_gradebook_with_pred["worst_model"] = unique_zero_gradebook.iloc[:,5:].idxmin(axis="columns")
        unique_zero_gradebook_with_pred["worst_model"] = unique_zero_gradebook_with_pred["worst_model"].apply(lambda x: x.split("_")[1])
        ...
        # (7) compute and save accuracy distribution 
