...
@registry.register_model("hotpot_net")
class HotpotNet(BaseModel):
    ...
    @classmethod
    def config_path(cls):
        return "configs/models/hotpot_net/defaults.yaml"
    ...
    def build(self):
        ...
        if self.config.use_lorra:
            self._build_cls_encoding()
    ...
    def _build_cls_encoding(self):
        self.cls_embedding = nn.Embedding(1, self.mmt_config.hidden_size)
    ...
    def _build_obj_encoding(self):
        self.remove_object_fasttext = self.config.obj.get("remove_object_fasttext", False)
        self.remove_object_frcn = self.config.obj.get("remove_object_frcn", False)
    ...
    def _build_mmt(self):
        if self.config.use_mmt:
            self.mmt = MMT(self.mmt_config, self.config.use_lorra)
        else:
            logger.info("="*10+"Not using MMT"+"="*10)
            self.mmt = SimpleFusionNet(self.mmt_config)
    ...
    def _forward_lorra(self, sample_list, fwd_results):
        # forward pass for LoRRA
        # LoRRA's forward pass is a bit different from the other models
        # (e.g. it does not use the MMT output as input to the classifier)
        ...
        cls_ids = torch.zeros((len(fwd_results["txt_inds"]), 1), dtype=torch.long).to(text_bert_out.device)
        cls_embed = self.cls_embedding(cls_ids)
        cls_mask = torch.ones((len(fwd_results["txt_inds"]), 1), dtype=torch.long).to(text_bert_out.device)

        mmt_results = self.mmt.forward_lorra(
        ...
        mmt_output = fwd_results["mmt_dec_output"].squeeze(1)
        fwd_results["scores"] = self.calculate_logits(mmt_output)
    ...
    def forward(self, sample_list):
        ...
        if not self.config.use_lorra:
            ...
            self._forward_lorra(sample_list, fwd_results)
    ...
    def _forward_obj_encoding(self, sample_list, fwd_results):
        # Object FastText feature (300-dim)
        object_fasttext = sample_list.object_feature_0
        object_fasttext = F.normalize(object_fasttext, dim=-1)
        assert object_fasttext.size(-1) == 300
        ...
        if self.remove_object_fasttext:
            object_fasttext = torch.zeros_like(object_fasttext)
        # obj_feat = obj_fc7
        obj_feat = torch.cat(
            [object_fasttext, obj_fc7], dim=-1
        )
        ...
        if not self.config.use_object:
            obj_mmt_in = torch.zeros_like(obj_mmt_in)
    ...
    def _forward_mmt(self, sample_list, fwd_results):
        ...
        if not self.config.use_question:
...
class MMT(BertPreTrainedModel):
    def __init__(self, config, use_lorra=False):
        super().__init__(config)
        if not use_lorra:
    ...
    def forward_lorra(
        ...
        cls_embed,
        cls_mask):
        encoder_inputs = torch.cat([txt_emb, obj_emb, ocr_emb, cls_embed], dim=1)
        attention_mask = torch.cat([txt_mask, obj_mask, ocr_mask, cls_mask], dim=1)
...
class SimpleFusionNet(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.prev_pred_embeddings = PrevPredEmbeddings(config)
        self.att_dec_question = BertSelfAttention(config.fusion_config)
        self.att_dec_object = BertSelfAttention(config.fusion_config)
        self.att_dec_ocr = BertSelfAttention(config.fusion_config)
        self.att_ocr_question = BertSelfAttention(config.fusion_config)
        self.att_ocr_object = BertSelfAttention(config.fusion_config)

    def get_inputs_and_mask(self, emb_1, mask_1, emb_2, mask_2, is_decode=False):
        emb_inputs = torch.cat([emb_1, emb_2], dim=1)
        attention_mask = torch.cat([mask_1, mask_2], dim=1)
        # offsets of each modality in the joint embedding space
        max_num_1 = mask_1.size(-1)
        max_num_2 = mask_2.size(-1)
        ...
        if is_decode:
            # decoding step elements can attend to themselves in a causal manner
            extended_attention_mask[:, :, -max_num_2:, -max_num_2:] = _get_causal_mask(
                max_num_2, emb_1.device
            )

        # flip the mask, so that invalid attention pairs have -10000.
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
        assert not extended_attention_mask.requires_grad
        return emb_inputs, extended_attention_mask
    ...
    def forward(
        ...
        dec_que_inputs, dec_que_attention_mask = self.get_inputs_and_mask(txt_emb, txt_mask, dec_emb, dec_mask,
                                                                          is_decode=True)
        dec_obj_inputs, dec_obj_attention_mask = self.get_inputs_and_mask(obj_emb, obj_mask, dec_emb, dec_mask,
                                                                          is_decode=True)
        dec_ocr_inputs, dec_ocr_attention_mask = self.get_inputs_and_mask(ocr_emb, ocr_mask, dec_emb, dec_mask,
                                                                          is_decode=True)
        ## for dec fusion
        dec_num = dec_emb.size(1)
        dec2question = self.att_dec_question(dec_que_inputs, dec_que_attention_mask)
        dec2obj = self.att_dec_object(dec_obj_inputs, dec_obj_attention_mask)
        dec2ocr = self.att_dec_ocr(dec_ocr_inputs, dec_ocr_attention_mask)
        mmt_dec_output = (dec2ocr[0][:, -dec_num:, :] + dec2obj[0][:, -dec_num:, :] + dec2question[0][:, -dec_num:, :])

        # for ocr fusion
        ocr_num = ocr_emb.size(1)
        ocr_que_inputs, ocr_que_attention_mask = self.get_inputs_and_mask(txt_emb, txt_mask, ocr_emb, ocr_mask)
        ocr_obj_inputs, ocr_obj_attention_mask = self.get_inputs_and_mask(obj_emb, obj_mask, ocr_emb, ocr_mask)

        ocr2question = self.att_ocr_question(ocr_que_inputs, ocr_que_attention_mask)
        ocr2obj = self.att_ocr_object(ocr_obj_inputs, ocr_obj_attention_mask)

        mmt_ocr_output = (ocr2question[0][:, -ocr_num:, :] + ocr2obj[0][:, -ocr_num:, :])
